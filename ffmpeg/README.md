...
这个文件夹是用来放 FFmpeg 可执行文件的，Bot 需要用它来处理媒体文件。

## 为什么这个文件夹是空的？

FFmpeg 的可执行文件体积太大了（约 170MB），不适合提交到 Git 仓库。所以请按照下面的步骤手动下载和配置——
...
1. **下载 FFmpeg **
   ...
   - 选择 `ffmpeg-master-latest-win64-gpl.zip` 这个文件
...
A: 当然可以的。如果你的系统 PATH 中已经有 ffmpeg 了，Bot 会自动使用它的；
...
**提示：** 这个文件夹中的 `*.exe` 和可执行文件已经在 `.gitignore` 中忽略了，不会被提交到版本控制，
