...
这份规范定义 AFC 框架内错误如何抛出、传递、捕获、记录——旨在给出一份区分用户错误、外部依赖错误、预期外错误的指南，确保日志可追溯、用户体验友好、测试友好。
...
- **所有异常都会记入日志**（`AFC 执行异常`）；executor 的 Exception 兜底会把原始异常消息 `str(e)` 直接透传进 `<FUNCTION_ERROR>{e}</FUNCTION_ERROR>` 返回给 LLM ——因此「不暴露敏感细节给 LLM」的责任完全落在工具入口层。密钥、内部路径等敏感信息必须在入口层就翻译干净，错误原因千奇百怪，不能指望 executor 兜底脱敏
...
**规则**：
- 触发词正则编译失败，则记 `AFC 触发词编译失败` 的日志（WARNING，经 `_logBadPattern`），跳过该条 pattern，继续扫描
- 工具 / 触发器导入失败（`ImportError` / `AttributeError`）在当前的实现为静默跳过、**不记日志**，继续扫描其它工具
- 被禁用工具（`isAfcToolEnabled() == False`）按设计静默跳过——这是开关行为，不算错误
