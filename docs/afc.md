...
- [设计决策](#设计决策)
  ...
  - [为什么 L1 和 L2 要并行执行而不是互斥](#为什么-l1-和-l2-要并行执行而不是互斥)
...
——若放在 group=2 与 LLM 同组，则两者并发执行，bot_data 写入时序不确定，可能工具上下文块注入晚于 LLM 读取。且在 [Telegram Handler 文档](./telegram-handlers-group.md) 中也告示过：
...
- **L2 正则**（与 L1 并行）：模式匹配，如 `\d+\s*[\+\-\*/]` 能匹配到 `calc`
- **L3 通用兜底**：命中"帮我"/"查一下"，但 L1/L2 都未能兜住，此时返回所有工具，让 LLM 自己选
- **L4 上下文延续**：短消息（≤10 字）+ 指代词（"那"/"再"/"也"），继承上一轮工具集

L1 和 L2 **并行执行**，结果取并集——这样即使 L1 已命中，L2 的正则也有机会补充遗漏的工具（详见下文设计决策）。

L3 则是"宁可过注入"的体现；L4 旨在支持多轮对话（"那明天（的天气）呢？" 继承上轮的 `weather`）。

### 为什么 L1 和 L2 要并行执行而不是互斥

早期设计中，L2（正则匹配）只在 L1（关键词匹配）未命中时执行——即「L1 有结果就跳过 L2」的互斥模式。这样设计的初衷是减少不必要的正则匹配开销，乍一看是没有什么问题的。

但实践中发现了问题：**当 L1 宽泛匹配导致误召回时，L2 的精准正则就失去了补救机会**。

例如：

> 消息："天气预报说明天温度 5+3 度"
> - L1 匹配："天气" → `weather`，命中后直接返回
> - L2 被跳过，无法匹配 `5+3` 正则 → `calc` 工具被漏掉

即使用户明确表达了计算意图（`5+3`），也会因为 L1 的宽泛匹配而被阻挡。

这个时候就需要**改为并行执行**了。让 L1 和 L2 独立运行，结果取并集。这样——
- L1 匹配 "天气" → `weather`
- L2 匹配 `5+3` → `calc`
- 最终返回 `{weather, calc}`，LLM 自主选择需要的工具

**为什么并行不会导致过度召回？**
1. **LLM 最终裁决**：即使多注入一两个工具 schema（每个约 50-200 token），LLM 仍会精准选择真正需要的工具，不会盲目调用无关工具
2. **符合"高召回率优先"哲学**：宁可多给 LLM 一个选项，也不应漏掉真实意图
3. **性能无忧**：L2 正则匹配是纯内存操作，无 I/O 或网络开销，耗时可忽略

这个改动在 2026-07 实施，代码见 [afcIntent.py::detectTools()](../utils/afc/afcIntent.py)。
...
顺着这个思路往下走，工具现在连「开关」和「身份名片」也归自己管了——`python scripts/afc_tools.py` 提供了一整套管理命令：`enable` / `disable` 切换启用状态，`install` / `uninstall` 装卸第三方工具，`update` 把第三方工具升级到新版本，`new` 一键生成脚手架，`check` 检查工具的状态，`list` / `show` 查看清单和详情。而工具叫什么名字、是干什么的，这些也不再写死在代码常量里，而是收在每个工具自带的 manifest（`afcTool.json`）里——也就是说，代码只负责埋头干活，名片上的事归名片管。

完整的管理细节单独成文，见 [docs/afc-tool-management.md](afc-tool-management.md)。
...
- JSON 解析失败 / 工具不存在 / 参数错误，则返回 `<FUNCTION_ERROR>…错误描述…</FUNCTION_ERROR>`
- 所有调用日志记录（成功/失败/超时）
...
对了，其实现在已经不用从空目录白手起家了——`python scripts/afc_tools.py new <tool>` 会一口气把 7 个文件的脚手架搭好（`__init__.py` / `core.py` / `triggers.py` / `config.py` / `README.md` / `afcTool.json` / 测试文件），照着里面的注释填就行……不过脚手架每块骨头是干什么的，还是得心里有数，所以，下面的步骤照样值得读完……

还有一点需要特别特别注意的是：**这是一个 afc 工具的最小实现方案** ——下面的例子为了快速上手，工具逻辑只有薄薄一层，一个 `core.py` 就能装下所有。实际开发复杂工具时，像 weather 有缓存、日期解析和 HTTP 客户端，该拆文件就拆(cache.py / dateParser.py / client.py)。 `__init__.py` 始终只留对外暴露的函数入口。作为参考，可以看到 calc、weather、datetime 都是这样写的。

……就是说，一个工具小到能在 `core.py` 塞下所有，不代表它就应该把所有东西都塞进里面的——！
...
touch utils/afc/tools/mytool/core.py
touch utils/afc/tools/mytool/triggers.py
```

每个工具自包含在一个目录下——这样以后要卸载或分享某个工具，整个目录直接拎走，不会有文件散落在项目各处。

### 2. 实现工具函数（薄壳 `__init__.py` + `core.py`）

现在的目录约定是**薄壳结构**：`__init__.py` 只做「对外窗口」——async 入口函数定义在它的顶层，函数体里转调 `core.py` 的实现；真正的工具逻辑全住在 `core.py` 里。

```python
"""
utils/afc/tools/mytool/__init__.py

我的工具描述（薄壳层，只定义入口，转调 core.py）
"""
...
async def myFunction(arg1: str, arg2: int = 10) -> str:
    ...
    返回：
        结果字符串
    """
    return await core.myFunction(arg1, arg2)

__all__ = ["myFunction"]
```

```python
# utils/afc/tools/mytool/core.py
"""mytool 实现层"""
...
**为什么不 `from .core import myFunction` 一把梭？** 因为管理脚本的 `check` 全程只做 AST 静态分析、绝不 import 工具代码（为什么见「安全边界」），它要在 `__init__.py` 顶层亲眼看到至少一个 `async def`，才承认这是个工具——纯 re-export 在 AST 眼里只是一条 import 语句，看不见入口。于是 calc 也从纯 re-export 改成了薄壳，和 weather / datetime 保持了一致的说。
...
- `__all__` 列出对外函数 — Python 的门面声明（`from tool import *` 时的导出清单），是好公民习惯；但它**不参与** registry 的过滤，防误扫靠的是下面这条叮嘱

还有一件事要特别叮嘱——**不应在 `__init__.py` 顶层随手写裸帮助函数**（非 `_` 开头、又不打算给 LLM 用的那种）。registry 扫描时不分青红皂白，会把它们一并当成工具、生成 schema 递给 LLM，那可就乱套了……内部帮手要么挪去 `core.py`，要么老老实实用 `_` 开头。
...
触发词写宽松一点没关系——它只是决定"要不要把这个工具的说明书递给 LLM"，多递几次不亏，真正用不用还是 LLM 自己拿主意。宁可让 LLM 看到了不调(浪费几十 token)，也别漏掉真实意图让它干瞪眼。

### 4. 登记 manifest（选择工具归属）

工具文件**不**登记进 `modulesRegistry.py`——工具的加载走目录扫描，不归模块注册表管。要登记的是工具的身份信息（显示名、描述、文件清单），按归属二选一：

- **内置工具**：把 manifest 加进仓库的 `data/afcToolsBuiltin.json`（照着 weather 的条目抄一份改改就行）；
- **第三方 / 可分发工具**：`afcTool.json` 留在工具目录里（脚手架已生成模板），随工具发布到 GitHub，安装方一句 `python scripts/afc_tools.py install <url>` 就能装上，manifest 会登记进 `data/afcToolsCustom.json`。

什么都没登记的工具也能跑——目录在 `utils/afc/tools/` 下就会被扫描发现，这种状态叫 local-draft：`list` / `show` 里显示名退化为目录名，`check` 会报一条 Warning 提醒它还没上户口。manifest 的字段规范详见 [docs/afc-tool-management.md](afc-tool-management.md)。
...
**拆文件的时机：** 当 `core.py` 超过 100-150 行，或者工具逻辑有明显的几块(HTTP 客户端、缓存、解析、格式化)，这就到了该拆文件的时候了。

参考 weather 的目录结构:

```
weather/
├── __init__.py          # 薄壳：只保留 getWeather 入口函数，转调各模块
├── triggers.py
├── config.py
...
├── test_toolManager.py     # 工具状态管理与启用开关
└── tools/
    ├── test_calc.py        # calc 工具单测
    ├── test_datetime.py    # datetime 工具单测
...
上面几道红线管的是「工具跑起来之后」的事；而 `afc_tools.py` 的 install / check 管道，管的是「工具还没被信任之前」的事——第三方仓库里拉下来的代码，在用户审阅前就是陌生人递来的包裹，直接拆开来跑，那可就太心大了……

所以这条管道立下了一条铁规矩：**全程只做 AST 静态分析，绝不 import 工具代码**。import 就会执行工具的顶层代码，等于包裹还没安检就先引爆了。`check` 的所有判断——`__init__.py` 顶层有没有 `async def`、`triggers.py` 的 KEYWORDS / PATTERNS 提不提得出来、正则编不编得过——全是从语法树上读出来的，一行工具代码都不会真的运行。

铁规矩之外，还有几道闸：

- **路径校验**：所有路径用 `realpath` + `commonpath` 比对基准目录（`_assertSafeToolPath`），而不是 `startswith` 子串匹配——`tools/../evil` 之类的小聪明混不过去。testFile 单独走 `tests/utils/afc/tools` 基准，且必须是扁平的 `test_<id>.py`。
- **下载限额**：单文件 ≤ 2MB、文件总数 ≤ 50、manifest ≤ 1MB、单次下载超时 15s——防的是有人塞个几个 G 的"工具"把磁盘撑爆。
- **原子落盘**：下载先落到同盘临时目录 `data/.afc_install`，全部齐了再 `os.replace` 原子换入——中途失败不会留下半个工具。
- **内置保护**：工具 id 撞上 `afcToolsBuiltin.json` 里的内置工具直接 abort，第三方包休想借壳上位；目标目录已存在时必须显式 `-f` 才覆盖。
- **uninstall 删前重新校验**：删除时不信任存下来的 `manifest.files`（万一它后来被改过呢），每个路径删除前重新过一遍路径校验，逃逸的直接 skip 并警告。

最后是一个容易混淆的边界问题：**bot 启动时 registry / afcIntent 明明也会 import 工具的 `__init__` 和 `triggers.py`，这和「绝不 import」矛盾吗？** 不矛盾——那是「工具已被信任之后」的正常加载，在信任边界之内；「绝不 import」专指管理管道在「尚未信任」阶段的克制。一个是检票入场后落座，一个是安检口外的包裹，这其实是两回事……
...
### 6. 运行时 import 与管道 import 是两回事

给管理脚本添功能时守住一条线：运行时的工具加载（`_scanTriggers` / `_scanTools` 在 bot 启动时 import 已安装工具）在信任边界以内，天经地义；管理管道（install / check / list）面对「尚未信任」的代码，**绝不 import**，一律走 AST 静态分析。缘由在「安全边界」一节已经讲透（细则见 [docs/afc-tool-management.md](afc-tool-management.md)）——可以说，觉得「import 一下更方便」的时候，就是该停下来想想的时候……
...
- [docs/afc-tool-management.md](afc-tool-management.md) — AFC 工具管理脚本（装卸 / 开关 / 更新 / 清单规范 / 安全校验）
