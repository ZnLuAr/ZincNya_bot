...
Memory 的查询方式是四层作用域分层过滤（global / chat / user / session）缩小检索范围，模型已经把单层数据量切到很小；同时每层最多取 3 条 (`perScopeLimit=3`)、全局总上限最多取 10 条 (`totalLimit=10`)，对一两百条记忆完全够用。
...
索引优先按作用域 `(scope_type, scope_id)`、启用状态 `enabled`、优先级 `priority DESC`、更新时间 `updated_at DESC` 排序，检索时不用全表扫描。
...
检索时按 scope 顺序依次拉取，每层上限 `perScopeLimit`（默认 3），总上限 `totalLimit`（默认 10）。

每层内部排序：`priority DESC → updated_at DESC → id DESC`
...
- (chat:123456, p=0, id=57, src=inferred) 这个群的话题主要是编程和动漫
</UNTRUSTED_MEMORY>
